...
# rally.cgi -
#
# Get/set EC-Rally configuration info for UI
#
...
################################
# getCfgList - Return the list of configurations for EC-Rally
#
# Arguments:
#   -
#
...
sub getCfgList {
    ...
    my $gcfg = new ElectricCommander::PropDB($::ec,"/projects/@PLUGIN_NAME@/rally_cfgs");
    ...
    foreach my $cfg (keys  %cfgs) {
        my $url = $gcfg->getCol("$cfg/rally_url");
        $xml .= "  <cfg>\n";
        $xml .= "     <name>$cfg</name>\n";
        $xml .= "     <rally_url>" . xmlQuote($url) . "</rally_url>\n";
        $xml .= "  </cfg>\n";
    }
    $xml .= "</cfgs>\n";
