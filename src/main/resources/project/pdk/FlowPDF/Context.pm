...
__PACKAGE__->defineClass({
    ...
    isMinimal               => FlowPDF::Types::Enum('1', '0')
});
...
sub new {
    ...
    # $self->setRunContext(buildRunContext($self->getEc()));
...
Note: sometimes it might be useful to take authScheme from other configuration parameter. In order to do that one need to use B<authSchemeProperty> parameter.
If this parameter is set its value will be used as a name of a property where auth scheme is stored.
...
Important note: to request modern rest client, "modern" parameter should be provided to this function:
...
%%%LANG=perl%%%
    ...
    # if you want to get the scheme from alternative location:
    my $rest = $context->newRestClient({
        modern => 1,
        baseUrl => 'http://localhost:8888',
        # this is the alternative location:
        authSchemeProperty => 'authType'
    });
...
sub newRESTClient {
    ...
    # defining auth scheme property that could go as default, from constants
    # or as authSchemeProperty parameter.

    my $authSchemeProperty = AUTH_SCHEME_PROPERTY;

    if ($params->{authSchemeProperty}) {
        $authSchemeProperty = $params->{authSchemeProperty};
    }
    ...
    if ($configValues) {
        ...
        my $authScheme = $configValues->getParameter($authSchemeProperty);
        if ($authScheme) {
        ...
        # handling ignoreSSL erorrs
        if (my $ignoreSSLErrors = $configValues->getParameter(IGNORE_SSL_ERRORS_PROPERTY)) {
            $creationParams->{ignoreSSLErrors} = $ignoreSSLErrors->getValue();
        }

        # ignoreSSLErrors could be supplied using creationParams also.
        # if so ignoreSSLErrors from configuration is replaced by direct value from the parameters.
        if ($params->{ignoreSSLErrors}) {
            $creationParams->{ignoreSSLErrors} = $params->{ignoreSSLErrors}->getValue();
        }
    }
    ...
    if ($useModernClient) {
        my $baseUrl = $configValues->{endpoint};
        if ($params->{baseUrl}) {
        ...
        unless ($baseUrl) {
            FlowPDF::Exception::RuntimeException->new(
                "baseUrl should be present in parameters field if morern client is being created. " .
                "It could be supplied using baseUrl parameter for newRESTClient method or be " .
                "available through the plugin configuration, in the endpoint field."
            )->throw();
        }
        $creationParams->{baseUrl} = $baseUrl;
    }
    unless ($useModernClient) {
        logWarning("Creation of REST client without baseURL as a parameter is deprecated and will be removed soon. Consider providing the baseUrl parameter.");
        $retval = FlowPDF::Client::REST->new($creationParams);
    }
    else {
        $retval = FlowPDF::Client::ModernREST->new($creationParams);
    }
