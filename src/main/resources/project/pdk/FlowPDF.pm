...
use FlowPDF::Log;
use FlowPDF::Log::FW;
use FlowPDF::EF::ExceptionForwarder;
...
sub newContext {
    my ($pluginObject, $opts) = @_;

    my $context = $pluginObject->getContextFactory()->newContext($pluginObject, $opts);
    return $context;
}
...
sub runStep {
    ...
    # build exception forwarder object
    my $exceptionForwarder = FlowPDF::EF::ExceptionForwarder->new({
        ec => ElectricCommander->new(),
        pluginObject => $flowpdf
    });
    ...
    my $context;
    try {
        $context = $flowpdf->getContext();
    } catch {
        my ($e) = @_;

        $exceptionForwarder->forward($e, {throw => 1});
    };
