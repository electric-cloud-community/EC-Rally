...
// RallyConfigListLoader.java --
//
// RallyConfigListLoader.java is part of ElectricCommander.
//
...
public class RallyConfigListLoader
    extends Loader
{

    //~ Instance fields --------------------------------------------------------

    private final RallyConfigList   m_configList;
    private final CgiRequestProxy m_cgiRequestProxy;
    private String                m_editorName;

    //~ Constructors -----------------------------------------------------------

    public RallyConfigListLoader(
            RallyConfigList   configList,
            Component       component,
            ...
            ChainedCallback callback)
    {
        ...
        m_cgiRequestProxy = new CgiRequestProxy(getPluginName(), "rally.cgi");
    }
    ...
    private void loadConfigs(Map<String, String> cgiParams)
    {
        ...
        try {
                    ...
                    new RequestCallback() {
                                ...
                                Throwable exception)
                        {
                            ((HasErrorPanel) m_component).addErrorMessage(
                                "Error loading Rally configuration list: ",
                                exception);
                        }
            ...
            if (m_component instanceof HasErrorPanel) {
                ((HasErrorPanel) m_component).addErrorMessage(
                    "Error loading Rally configuration list: ", e);
            }
            else {
                m_component.getLog()
                           .error(e);
    ...
    private void loadEditors()
    {
        ...
        request.setPropertyName("/plugins/EC-Rally/project/ui_forms/"
                + m_editorName);
        request.setExpand(false);
        request.setCallback(new EditorLoaderCallback("rallycfg"));
        ...
        implements PropertyCallback
    {
        ...
        @Override public void handleResponse(Property response)
        {
            ...
            String errorMsg = "Editor '" + m_editorName
                    + "' not found for Rally plugin '" + m_configPlugin + "'";
